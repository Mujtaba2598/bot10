...
class AITradingEngine {
    ...
    calculatePositionSize(initialInvestment, currentProfit, targetProfit, timeElapsed, timeLimit, confidence) {
        ...
        const baseSize = initialInvestment * 0.1;
...
app.post('/api/startTrading', (req, res) => {
    ...
    res.json({ 
        success: true, 
        botId, 
        message: `1-HOUR TARGET ACTIVE: $${parseFloat(targetProfit).toLocaleString()} target` 
    });
});
...
app.post('/api/tradingUpdate', (req, res) => {
    ...
    if (timeRemaining > 0 && Math.random() > 0.4) {
        ...
        if (signal.action !== 'HOLD') {
            ...
            const quantity = (positionSize / marketPrice).toFixed(4);
